...
# We test that the determinants of the following three matrices coincide:
...
# 3) the per-transposed extended Jacobi-Trudi matrix with powers la[N-1],...,la[0] on the main diagonal.
...
def test_schur_via_jacobi_trudi_three_forms(la, N, verbose):
    xs = pol_vars('x', N)
    ...
    r2 = my_det(A2)
    if verbose:
        print('test_schur_via_jacobi_trudi_three_forms')
...
def big_symbolic_test_schur_via_jacobi_trudi_three_forms(lambda_sum_max, N_max):
    print('big_symbolic_test_schur_via_jacobi_trudi_three_forms,')
    ...
    big_result = True
    for la, N in tests:
        result = test_schur_via_jacobi_trudi_three_forms(la, N, False)
...
print(big_symbolic_test_schur_via_jacobi_trudi_three_forms(12, 6))


# the following test takes more than 3 hours.
#print(big_symbolic_test_schur_via_jacobi_trudi_three_forms(16, 8))
